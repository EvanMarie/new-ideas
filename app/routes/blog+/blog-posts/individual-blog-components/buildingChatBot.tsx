...
import {
  blogHeadingStyles,
  blogLinkStyles,
  blogParagraphCodeStyles,
  blogSectionCodeStyle,
} from "../../components/blogItemRendering";
import CenterHorizontalFull from "~/components/buildingBlocks/centerHorizontalFull";

export default function BuildingChatbot() {
  return (
    <VStackFull className="" align="items-start" gap="gap-[2vh]">
      <p className={paragraphTextSize}>
        ...
        <a
          className={blogLinkStyles}
          href="https://github.com/DarkViolet-ai/remix-simple-chat"
        >
          GitHub
        </a>
        ...
        article. Nor will I go in depth on the UI components, though I encourage
        you to check out the{" "}
        <a
          className={blogLinkStyles}
          href="git@github.com:EvanMarie/blank_remix_tailwind_template.git"
        >
          template project
        </a>{" "}
        that I used, by{" "}
        <a className={blogLinkStyles} href="https://github.com/EvanMarie">
          @EvanMarie
        </a>
        .
      </p>

      <h1 className={blogHeadingStyles}>Getting Started</h1>

      <p className={paragraphTextSize}>
        Clone the project and do{" "}
        <code className={blogParagraphCodeStyles}>npm install</code>. You can
        now run the project with{" "}
        <code className={blogParagraphCodeStyles}>npm run dev</code>.
      </p>

      <h1 className={blogHeadingStyles}>The Simplest Chat Bot</h1>
      ...
      <p className={paragraphTextSize}>
        ...
        langchain, as its integrations do reduce the amount of boilerplate code
        we need to write. In this project, you can easily switch between
      ...
      <p className={paragraphTextSize}>
        For DeepInfra, there is no built-in integration for langchain. Instead,
        this is included as a util in this project. You can see the code for
        this in{" "}
        <code className={blogParagraphCodeStyles}>
          /app/lib/server-utils/deepInfraLangChain.ts
        </code>
        . (Note that as of this writing, there is an error in the langchain code
        that needs to be fixed in order for the deepInfra code to work. I have
        submitted a pull request for this very simple fix, and I expect it to be
        merged soon. If you get errors, please stick to the OpenAI and
        TogetherAI integrations for now.)
      </p>

      <p className={paragraphTextSize}>
        The route for this first implementation is at /chat, and the code can be
        found in{" "}
        <code className={blogParagraphCodeStyles}>
      ...
      <CenterHorizontalFull>
        <pre className={blogSectionCodeStyle}>
      ...
      <p className={paragraphTextSize}>
        This code is pretty simple. It uses the{" "}
        <code className={blogParagraphCodeStyles}>chatLoader</code> to get the
        messages from the server, and then passes them to the{" "}
        <code className={blogParagraphCodeStyles}>ChatComponent</code> to render
        them. Meanwhile, inside the chat component is a form that results in a
        POST request to the server, which is answered by the{" "}
        <code className={blogParagraphCodeStyles}>chatAction</code> above. The{" "}
        <code className={blogParagraphCodeStyles}>chatAction</code> is defined
        in{" "}
        <code className={blogParagraphCodeStyles}>
          /app/routes/chat/actions/chatAction.ts
        </code>{" "}
        and looks like this:
      </p>
...
export const chatAction = async ({ request }: ActionFunctionArgs) => {
  ...
  return await setSessionIdOnResponse(responseJson, sessionId);
      ...
      <p className={paragraphTextSize}>
        ...
        <code className={blogParagraphCodeStyles}>chatAction</code> function is
        an async function that takes an{" "}
        <code className={blogParagraphCodeStyles}>ActionFunctionArgs</code>{" "}
        object as an argument. This object contains the request object, which we
        use to get the form data from the POST request. We then create a new{" "}
        <code className={blogParagraphCodeStyles}>HumanMessage</code> object
        from the chat input, and add it to the message history.
      </p>

      <p className={paragraphTextSize}>
        We are using another langchain integration that uses redis as the
        backing store for our messages. This is very fast and convenient. All we
        have to do is provide a key to the chat conversation, which we get from
        the sessionID found in the cookie, or we create one if there is not one
        already. We append the new message to the history, and then retrieve the
        full history from redis.
      </p>
      ...
      <p className={paragraphTextSize}>
        ...
        to the message history, returning the response as a JSON object. We also
        set the session ID on the response, so that the session ID is stored in
        the cookie for the next request.
      </p>

      <p className={paragraphTextSize}>
        After the action completes, remix revalidates all of the loaders, and so
        our loader only needs to retrieve the message history from redis and
        return it. This is done in the{" "}
        <code className={blogParagraphCodeStyles}>chatLoader</code> function,
        which is defined in{" "}
        <code className={blogParagraphCodeStyles}>
          /app/routes/chat/loaders/chatLoader.ts
        </code>{" "}
        and looks like this:
      </p>
...
export type ChatLoaderData = {
  messages: { type: "ai" | "human"; text: string; timestamp: string }[];
      ...
      <p className={paragraphTextSize}>
        ...
        chat history for that conversation and start a new chat. The loader also
        retrieves the session ID from the cookie, or creates a new one if it
        does not exist. It then retrieves the message history from redis, and if
        there are no messages, it adds a welcome message to the history. The
        messages are then returned as a JSON object, along with the session ID
        set on the response.
      </p>

      <p className={paragraphTextSize}>
        And that's it! You now have a simple chat bot that will respond to every
        statement the user makes. You can easily switch between different LLM
        service providers by changing the{" "}
        <code className={blogParagraphCodeStyles}>chatModel</code> variable in
        the <code className={blogParagraphCodeStyles}>chatAction</code>{" "}
        function. This is a great starting point for building more complex chat
        bots with Remix.
      </p>

      <p className={paragraphTextSize}>
        In the{" "}
        <a href="/blog/35" className={blogLinkStyles}>
          next article
        </a>
        , we will show you how to build a more advanced chat bot that implements
        RAG (Retrieval Augmented Generation) and other features.
      </p>
    </VStackFull>
  );
}
